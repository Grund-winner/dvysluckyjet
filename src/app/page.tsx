...
function getConsecutive(): { cat: string | null; count: number } {
  const mem = loadMemory();
  const preds = mem.predictions;
  if (!preds.length) return { cat: null, count: 0 };
  const last = preds[preds.length - 1].category;
  let count = 1;
  for (let i = preds.length - 2; i >= 0; i--) {
    if (preds[i].category === last) count++;
    else break;
  }
  return { cat: last, count };
}
...
export default function Home() {
  ...
  const displayPrediction = useCallback((data: PredictionData) => {
    ...
    if (pred >= 10.0) {
      setPredValColor('#ff9800');
      setPredValShadow('0 0 30px rgba(255,152,0,.5),0 0 60px rgba(255,152,0,.2)');
    } else if (pred >= 2.0) {
      setPredValColor('#b388ff');
      setPredValShadow('0 0 25px rgba(179,136,255,.5),0 0 50px rgba(179,136,255,.2)');
    } else {
      setPredValColor('#64b5f6');
      setPredValShadow('0 0 25px rgba(100,181,246,.4),0 0 50px rgba(100,181,246,.15)');
    }
  ...
  const doPredict = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === 'ok') {
        // Validate last prediction with latest result
        if (data.last_rounds && data.last_rounds.length > 0) {
          validateLastPrediction(data.last_rounds[0]);
        }

        // Update memory with merged history
        if (data.history) {
    ...
    const initApp = async () => {
      try {
        ...
        if (data.authenticated) {
          ...
          if (acc !== null) {
            showToast(msg + ' (' + acc + '% precision)', 3500);
          } else {
            showToast(msg, 3500);
          }
    ...
    <div className="app">
      <div className="game-area" ref={gameAreaRef}>
        ...
        <div className="ov-top">
          ...
          <img
            className="ov-logo"
            src="https://1play.gamedev-tech.cc/lucky_grm/assets/media/3caa6b7c2d37c3ae0bd198c86b81bb13.svg"
            alt="Lucky Jet"
          />
        </div>
        ...
        <div className="ov-bottom">
          ...
          <div className="pred-section">
            ...
            <div className="pred-subrow">
              {signals.map((sig, i) => (
                <span key={i} className={`sig ${sig.type}`}>
                  {sig.text}
                </span>
              ))}
            </div>
      ...
      {/* Toast */}
      <div ref={toastRef} className={`toast ${toastVisible ? 'show' : ''}`}>
        {toastMsg}
      </div>
    </div>
  );
